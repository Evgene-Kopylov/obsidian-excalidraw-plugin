...
| Extract font management | Implemented; awaiting manual validation | `FontManager` now owns CJK discovery/loading, document-scoped custom-font registration, readiness, and cleanup; plugin methods and the externally read `fourthFontLoaded` field remain intact |
| Remove avoidable native element creation | Complete | Five detached image/canvas operations now obtain their typed elements from an Obsidian `DocumentFragment`; the two settings-to-Markdown paths pass fragments directly to `htmlToMarkdown()` and perform required heading conversion on cloned DOM; `FontManager` registers only its known CJK/local faces as document-scoped `FontFace` objects with explicit replacement and cleanup. The historical `addFonts(string[])` delegate and its first-pass CSSOM parser were removed after confirming they were neither part of the package entrypoint nor used or documented externally. CJK subsets now retain their source `unicodeRange` descriptors. The seven non-iframe dynamic stylesheet sites and five iframe-owned stylesheet sites remain deliberately unchanged; focused manual testing passed |
...
| Preserve local Markdown images across drawing copy/paste and deletion | Implemented; base copy/paste fix manually validated; follow-up pending | A local Markdown image's element metadata now takes precedence over stale ordinary-image registry entries during scene synchronization. Paste copies available back-of-note bodies into the destination before its delayed save, and loading a damaged drawing no longer erases the plugin-wide source needed by other open drawings. Deletion detection and Markdown-image classification no longer perform restoring lookups as incidental side effects. Incompatible registry cleanup occurs inside an already-authorized save and does not request `loadDrawing()` for unchanged scene files. Clean sidepanel selection changes detach the prior editor without force-saving or reloading every scene image; pending content and appearance changes retain their existing render/persistence path. The confirmation can persist **Keep** or **Delete** for future deletions, with an **Ask every time** reset in both settings UIs |
| Make local Markdown-image/embeddable conversion transactional | Complete; manually validated | Conversion is serialized, reads the current Canvas-node fragment, waits for its automatic Vault persistence only when an active edit differs from disk, and verifies a unique section before changing either the scene or note. Image-to-embeddable conversion unwraps the owned marker block before inserting the section and keeps exactly one empty level-one heading when it is the final visible boundary before commented Excalidraw data. Reverse conversion removes that complete section boundary, rejects reserved markers or over-captured headings, and commits the staged note change only after the element conversion succeeds. The sidepanel tracks the selected Markdown-image identity rather than only the stable element ID, so it clears and refreshes across retyping |
| Validate Markdown-image dependencies and preserve conversion appearance | Implemented; awaiting manual validation | Focus validation now includes directly referenced external Markdown sources even when their runtime `EmbeddedFile.mtime` was never initialized. Nested drawing cache validation uses the existing dependency traversal with non-Excalidraw vault files included, so a Markdown source change invalidates every containing drawing cache. A command forces fresh generation for one selected image or all scene images, bypassing and replacing applicable persistent entries. Markdown-image custom data remains on an intermediate embeddable and supplies the restored appearance during reverse conversion |
...
| --- | --- | --- | --- |
| 2026-09-06 | Extended Markdown-image cache validation and conversion metadata retention | Reused the existing nested dependency traversal with non-Excalidraw vault files enabled, removed the stale runtime-mtime gate from direct focus validation, added a selected-or-all scene image refresh command whose forced load bypasses and replaces persistent cache entries, and retained Markdown-image custom data on the intermediate embeddable so reverse conversion restores its appearance. No dependency index or targeted IndexedDB deletion layer was added | Node 22.22.2 production and library builds and bundle syntax pass with the established 33 circular warnings. Focused ESLint, unused-symbol lint, and `git diff --check` pass; full source lint retains its existing 120-error backlog with no finding in touched files. Manual validation should change an external Markdown source while its drawing is inactive, test the same dependency through an embedded drawing after a cold reopen, invoke the refresh command with exactly one image and with zero/multiple selected, and round-trip a styled local and external Markdown image through an embeddable. Main-window coverage is required; repeat dependency refresh in a popout and run one mobile smoke test because cache storage and focus timing differ |
| 2026-09-06 | Added view-local render-input caching for Markdown images | `ViewSceneFileManager` now retains the exact Markdown and one configuration signature containing the source path, resolved appearance, and Markdown-image custom data used for each displayed image. `EmbeddedFilesLoader` skips SVG regeneration only when those inputs still match and the current Excalidraw API still holds the rendered file; content, style/default, width, source, or metadata changes regenerate normally, as do explicit forced refreshes and cache misses after an API scene reset. Successful insert, duplicate, edit, and embeddable-conversion renders seed the same cache, and obsolete entries are pruned against current scene membership. No persistent image cache or file-mtime heuristic was introduced | Node 22.22.2 production build and bundle syntax pass with the established 33 circular warnings. Focused lint and unused-symbol scan pass outside the existing `ExcalidrawView.ts` backlog; final diff checks pass. Manual validation pending: open a drawing with several local and external Markdown images, trigger repeated full scene reloads and convert an embeddable, and confirm only the new/changed image refreshes without flicker. Then change Markdown text, width, each appearance setting, plugin defaults used by a legacy/partial element, and an external source, confirming each affected image updates once. Exercise explicit refresh, a same-tab file switch, main window, popout, and one mobile reload smoke test |
| 2026-09-06 | Made Markdown-image/embeddable conversion transactional after live race diagnostics | The prior conversion compared unrelated whole-file snapshots and then removed a fragment with exact string replacement. Live traces exposed three independent hazards: duplicate conversion requests, Canvas-node edits not yet persisted, and a missing visible heading boundary causing Obsidian to transclude the commented Excalidraw payload recursively. Conversion now runs one-at-a-time, validates the live fragment against a unique persisted range, waits for the Canvas node's automatic Vault write when necessary, rejects managed or over-captured content, unwraps the marker block before reinserting a local section, and preserves exactly one structural `#` only when the final Markdown-image scaffold disappears. The false warning for a valid single level-one heading is gone. The sidepanel now observes Markdown-image identity across same-ID element retyping. After validation, all temporary logging was removed and the production helpers were reduced to one range resolver, one boolean safety predicate, and one shared conversion guard; no generic parser, temporary `ExcalidrawData`, Canvas save override, or additional persistence layer remains | User validated ordinary round trips, multiple images, recovery after correcting an already-malformed boundary, and immediate edit-then-convert behavior. Focused lint, production build, bundle syntax, unused-symbol scan, debug-prefix search, and diff checks pass |
| 2026-09-05 | Hardened local Markdown-image copy/paste and deletion behavior from live diagnostics | The copy payload contained all three Markdown bodies, but `syncFiles()` short-circuited on stale ordinary-file entries and skipped Markdown restoration. Reloading the incomplete destination then deleted the shared Markdown source, poisoning later pastes. Local element metadata is now authoritative, paste hydrates the destination registry before save, and a malformed/incomplete drawing cannot delete another view's shared source. Deletion checks no longer call the restoring lookup, eliminating the repeated-confirmation loop. Follow-up testing exposed selection flicker: a clean editor switch still forced save/reload, while Markdown-image classification first restored a stale ordinary file. Clean switches now only detach; real content/appearance changes retain the guarded persistence path. Added an optional confirmation toggle backed by a validated `ask`/`keep`/`delete` setting, with the exact setting name shown in the dialog, and documented the changes in 2.27.3 release notes. Audit against `4e7f4699` confirms its parse-before-mutation safety boundary remains intact: incoming Drawing JSON is still fully parsed and validated before `loaded`, registries, file identity, or scene are changed; the Markdown master change occurs only after successful parse and removes a cross-drawing side effect. Removed all temporary text diagnostics | Node 22.22.2 production and library builds and `node --check dist/main.js` pass with the established circular warnings; focused ESLint, unused-symbol lint, and `git diff --check` pass. The temporary debug prefix is absent from `src/` and rebuilt `dist/main.js`. Full source lint retains its existing 120-error backlog with no finding in newly changed lines; TypeScript retains only its existing generated-doc, library-output, and dependency declaration failures. The base copy/paste case is manually validated. Follow-up validation pending: switch repeatedly among unchanged local/external Markdown images with the editor open and confirm no whole-scene flicker or disk save; edit Markdown and appearance immediately before switching and confirm render/save still completes; test keyboard and element-panel deletion for keep/delete, remembered batch/future behavior, settings reset, undo/redo, main/popout, and one mobile prompt/settings smoke test |
| 2026-09-05 | Closed the avoidable native-element-creation checkpoint | Retained the fragment, direct-DOM serialization, and document-scoped `FontFace` solution after the full risk-based manual test pass; documented the accepted patterns and narrowed the Rollup shim rationale to the 12 remaining reviewed style-element cases. No dynamic stylesheet creation was changed | User confirmed all requested font, settings-Markdown, cropped/exported image, screenshot, canvas-snapshot, popout/mobile, and scanner validation passed successfully |
| 2026-09-05 | Removed the eight approved `deliberateCreateElement` cases without changing dynamic stylesheet creation | Replaced five detached canvas/image creations with `createFragment().createEl()`, removed the now-unneeded owner-document argument from EA view-PNG cropping, converted the two temporary settings-serialization containers to direct `DocumentFragment` input (with DOM-based heading conversion for the legacy exporter), and replaced `FontManager`'s generated `@font-face` style element with document-owned `FontFace` registrations. A follow-up API audit rejected the first-pass CSSOM parser: `addFonts(string[])` had no source consumer, documentation, or package-entrypoint export and was an accidental historical public method, so its plugin/manager delegates and parser were removed. `getCJKFontFaceData()` now keeps each data URL paired with its original `unicodeRange`; CJK faces remain lazy and the user-selected fourth font retains eager readiness. No `CSSStyleSheet` constructor dependency remains. No `Messages.ts` entry was added. All seven non-iframe dynamic style-element sites and all five iframe-owned style-element sites are untouched | Node 22.22.2 production and library builds pass with the established 33 production circular warnings; the simplified production bundle is 4,872,998 bytes. `node --check dist/main.js`, focused ESLint, unused-symbol lint, and `git diff --check` pass. Repository TypeScript retains its established generated-doc/library/dependency declaration failures and reports no changed-source diagnostic. Generated declarations contain neither `addFonts()` nor the removed parser. The source inventory fell from 20 to 12 `deliberateCreateElement()` calls, all of which create `style` elements: seven intentionally excluded non-iframe sites and five deferred iframe sites. Manual validation pending: local/CJK font registration and cleanup across main/popout documents; declarative and legacy settings Markdown copy fidelity; EA cropped view PNG, crop-mask PNG, desktop screenshot stitching, and Markdown canvas snapshot export, with popout/mobile coverage where applicable |
